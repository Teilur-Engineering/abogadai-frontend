...
export default function Sidebar() {
  ...
  const { haySessionActiva, abandonarSesion, isAbandonando } = useSession();

  const [isExpanded, setIsExpanded] = useState(true);
  const [tieneNovedades, setTieneNovedades] = useState(false);

  // Estado para el modal de confirmación de navegación
  const [mostrarModalNavegacion, setMostrarModalNavegacion] = useState(false);
  ...
  /**
   * Intercepta la navegación y muestra modal si hay sesión activa
   */
  const handleNavClick = (e, path) => {
    ...
    // Si hay sesión activa y NO estamos navegando a /app/avatar, mostrar modal
    if (haySessionActiva() && path !== '/app/avatar') {
      e.preventDefault();
      setRutaPendiente(path);
      setMostrarModalNavegacion(true);
    }
    // Si no hay sesión activa, dejar que NavLink navegue normalmente
  };

  /**
   * Cancela la navegación: cierra modal y se queda
   */
  const handleCancelarNavegacion = () => {
    setMostrarModalNavegacion(false);
    setRutaPendiente(null);
  };

  /**
   * Maneja click en perfil (interceptar si hay sesión activa)
   */
  const handlePerfilClick = () => {
    if (haySessionActiva()) {
      setRutaPendiente('/app/perfil');
      setMostrarModalNavegacion(true);
    } else {
      navigate('/app/perfil');
    }
  ...
  const handleLogout = () => {
    // Si hay sesión activa, mostrar modal primero
    if (haySessionActiva()) {
      setRutaPendiente('__logout__');
      setMostrarModalNavegacion(true);
    } else {
      logout();
      navigate('/login');
  ...
  /**
   * Confirma la navegación especial (incluyendo logout)
   */
  const handleConfirmarNavegacionEspecial = async () => {
    const exito = await abandonarSesion();
    if (exito) {
      setMostrarModalNavegacion(false);
      if (rutaPendiente === '__logout__') {
        logout();
        navigate('/login');
  ...
  return (
    <>
      ...
      {/* Modal de confirmación para navegación durante sesión activa */}
      <ModalConfirmarSalida
        isOpen={mostrarModalNavegacion}
        onConfirmar={handleConfirmarNavegacionEspecial}
        onCancelar={handleCancelarNavegacion}
        titulo="¿Salir de la sesión actual?"
        mensaje="Tienes una sesión activa en curso. Si navegas a otra página, perderás todo el progreso y el caso será eliminado permanentemente."
        textoConfirmar="Salir y eliminar caso"
        textoCancelar="Volver a la sesión"
        isLoading={isAbandonando}
      />
    </>
  );
}
